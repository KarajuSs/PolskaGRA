...
public class CoastConveyerNPC implements ZoneConfigurator  {
	...
	private StendhalRPZone getIslandDockZone() {
		if (islandDocksZone == null) {
			islandDocksZone = SingletonRepository.getRPWorld().getZone("0_warszawa_ne");
		}
	...
	private void buildNPC(StendhalRPZone zone) {
		final SpeakerNPC npc = new SpeakerNPC("Basia") {
							...
							public void fire(final Player player, final Sentence sentence, final EventRaiser npc) {
								switch (ferryState) {
									case ANCHORED_AT_GDANSK:
										npc.say("Czy chcesz, abym zabrał Ciebie do Gdańska?");
							...
							public void fire(final Player player, final Sentence sentence, final EventRaiser npc) {
								switch (ferryState) {
									case ANCHORED_AT_GDANSK:
										...
										break;
									case ANCHORED_AT_WARSZAWA:
										player.teleport(getIslandDockZone(), 24, 92, Direction.LEFT, null);
										npc.setCurrentState(ConversationStates.IDLE);
										break;
									default:
	...
	private static StendhalRPZone getMainlandDocksZone() {
		if (mainlandDocksZone == null) {
			mainlandDocksZone = SingletonRepository.getRPWorld().getZone("0_gdansk_nw");
		}
